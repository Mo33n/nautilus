...
// 2. SIMD IMPLEMENTATION (f32x8)
impl BlackScholesReal for f32x8 {
    type Mask = Self;
    #[inline(always)]
    fn splat(val: f64) -> Self {
    ...
    #[inline(always)]
    fn cmp_gt(self, other: Self) -> Self::Mask {
        <Self as wide::CmpGt>::simd_gt(self, other)
    }
    #[inline(always)]
    fn mul_add(self, a: Self, b: Self) -> Self {
        self.mul_add(a, b)
    ...
    fn recip_precise(self) -> Self {
        let est = self.recip();
        est * (Self::splat(2.0) - self * est) // Newton-Raphson refinement for 1e-7
    }
    ...
    fn cdf_with_pdf(self) -> (Self, Self) {
        let abs_x = self.abs();
        let t = Self::splat(1.0) / (Self::splat(1.0) + Self::splat(0.2316419) * abs_x);

        // Horner's method with FMA
        let mut poly = Self::splat(1.330_274_5);
        poly = t.mul_add(poly, Self::splat(-1.821_255_9));
        poly = t.mul_add(poly, Self::splat(1.781_477_9));
        poly = t.mul_add(poly, Self::splat(-0.356_563_78));
        poly = t.mul_add(poly, Self::splat(0.319_381_54));

        let pdf = Self::splat(0.398_942_3) * (Self::splat(-0.5) * self * self).exp();
        let res = Self::splat(1.0) - pdf * (poly * t);
        (
            <Self as wide::CmpGt>::simd_gt(self, Self::splat(0.0))
                .blend(res, Self::splat(1.0) - res),
            pdf,
        )
    }
}

// 3. SCALAR IMPLEMENTATION (f32) - Manual Minimax for 1e-7 Precision
...
// 3b. SCALAR IMPLEMENTATION (f64) - Optimized for high precision
impl BlackScholesReal for f64 {
    type Mask = bool;
    #[inline(always)]
    fn splat(val: f64) -> Self {
        val
    ...
    #[inline(always)]
    fn ln(self) -> Self {
        // Optimized ln for f64 using bit manipulation and polynomial approximation
        let bits = self.to_bits();
        let exponent = ((bits >> 52) as i32 - 1023) as Self;
        let mantissa = Self::from_bits((bits & 0x000FFFFFFFFFFFFF) | 0x3FF0000000000000);
        let x = (mantissa - 1.0) / (mantissa + 1.0);
        let x2 = x * x;
        // Higher precision polynomial for f64
        let mut res = 0.239_282_857_013_180_1_f64;
        res = x2.mul_add(res, 0.285_182_361_122_893_7);
        res = x2.mul_add(res, 0.4000058789795272);
        res = x2.mul_add(res, 0.6666666666666632);
        res = x2.mul_add(res, 2.0);
        x.mul_add(res, exponent * std::f64::consts::LN_2)
    }

    #[inline(always)]
    fn exp(self) -> Self {
        // Optimized exp for f64 using bit manipulation and polynomial approximation
        let k = (self.mul_add(
            std::f64::consts::LOG2_E,
            if self > 0.0 { 0.5 } else { -0.5 },
        )) as i32;
        let r = self - (k as Self * std::f64::consts::LN_2) - (k as Self * 1.4286068203094172e-12);
        // Higher precision polynomial for f64
        let mut res = 0.000_198_412_698_412_698_4_f64;
        res = r.mul_add(res, 0.001388888888888889);
        res = r.mul_add(res, 0.008333333333333333);
        res = r.mul_add(res, 0.041666666666666664);
        res = r.mul_add(res, 0.16666666666666666);
        res = r.mul_add(res, 0.5);
        res = r.mul_add(res, 1.0);
        r.mul_add(res, 1.0) * Self::from_bits(((k + 1023) as u64) << 52)
    }
    ...
    #[inline(always)]
    fn cdf_with_pdf(self) -> (Self, Self) {
        // Optimized CDF/PDF for f64 using polynomial approximation
        let abs_x = self.abs();
        let t = 1.0 / (1.0 + 0.2316419 * abs_x);
        // Higher precision polynomial coefficients for f64
        let mut poly = 1.330274429_f64;
        poly = t.mul_add(poly, -1.821255978);
        poly = t.mul_add(poly, 1.781477937);
        poly = t.mul_add(poly, -0.356563782);
        poly = t.mul_add(poly, 0.319381530);
        let pdf = 0.3989422804014327 * (-0.5 * self * self).exp();
        let res = 1.0 - pdf * (poly * t);
        (if self > 0.0 { res } else { 1.0 - res }, pdf)
    }
}

// 4. DATA STRUCTURES & CORE KERNEL
...
impl OptionDataSoA {
    /// # Panics
    ///
    /// Panics if `guesses` slice length is insufficient for the batch size.
    pub fn batch_iv(&self, guesses: &[f32]) -> Vec<Greeks<f32>> {
        let mut res = vec![Greeks::default(); self.s.len()];
        res.par_chunks_mut(1024)
            .enumerate()
            .for_each(|(ci, chunk)| {
                let base = ci * 1024;
                let mut i = 0;
                while i + 8 <= chunk.len() {
                    let idx = base + i;
                    let initial_v = f32x8::new(guesses[idx..idx + 8].try_into().unwrap());
                    let r_vec = f32x8::new(self.r[idx..idx + 8].try_into().unwrap());
                    let g_v = compute_iv_and_greeks(
                        f32x8::new(self.mkt[idx..idx + 8].try_into().unwrap()),
                        f32x8::new(self.s[idx..idx + 8].try_into().unwrap()),
                        f32x8::new(self.k[idx..idx + 8].try_into().unwrap()),
                        f32x8::new(self.t[idx..idx + 8].try_into().unwrap()),
                        r_vec,
                        r_vec, // Use r as b for backward compatibility
                        BlackScholesReal::cmp_gt(
                            f32x8::new(self.is_call[idx..idx + 8].try_into().unwrap()),
                            f32x8::splat(0.5),
                        ),
                        initial_v,
                    );
                    self.unpack(chunk, i, g_v);
                    i += 8;
                }
                while i < chunk.len() {
                    let idx = base + i;
                    let r_val = self.r[idx];
                    chunk[i] = compute_iv_and_greeks(
                        self.mkt[idx],
                        self.s[idx],
                        self.k[idx],
                        self.t[idx],
                        r_val,
                        r_val, // Use r as b for backward compatibility
                        self.is_call[idx] > 0.5,
                        guesses[idx],
                    );
                    i += 1;
                }
    ...
    /// # Panics
    ///
    /// Panics if `sigmas` slice length is insufficient for the batch size.
    pub fn batch_greeks(&self, sigmas: &[f32]) -> Vec<Greeks<f32>> {
        let mut res = vec![Greeks::default(); self.s.len()];
        res.par_chunks_mut(1024)
            .enumerate()
            .for_each(|(ci, chunk)| {
                ...
                while i + 8 <= chunk.len() {
                    let idx = base + i;
                    let r_vec = f32x8::new(self.r[idx..idx + 8].try_into().unwrap());
                    let g_v = compute_greeks(
                        f32x8::new(self.s[idx..idx + 8].try_into().unwrap()),
                        f32x8::new(self.k[idx..idx + 8].try_into().unwrap()),
                        f32x8::new(self.t[idx..idx + 8].try_into().unwrap()),
                        r_vec,
                        r_vec, // Use r as b for backward compatibility
                        f32x8::new(sigmas[idx..idx + 8].try_into().unwrap()),
                        BlackScholesReal::cmp_gt(
                            f32x8::new(self.is_call[idx..idx + 8].try_into().unwrap()),
                            f32x8::splat(0.5),
                        ),
                ...
                while i < chunk.len() {
                    let idx = base + i;
                    let r_val = self.r[idx];
                    chunk[i] = compute_greeks(
                        self.s[idx],
                        self.k[idx],
                        self.t[idx],
                        r_val,
                        r_val,
                        sigmas[idx], // Use r as b for backward compatibility
                        self.is_call[idx] > 0.5,
                    );
                    i += 1;
                }
    ...
    #[inline(always)]
    fn unpack(&self, chunk: &mut [Greeks<f32>], i: usize, g_v: Greeks<f32x8>) {
        let (p, v, d, gm, vg, th) = (
            g_v.price.to_array(),
            g_v.vol.to_array(),
            g_v.delta.to_array(),
